...
/**
 * 用户TidyExecutor执行的抽象任务
 *
 * @param <U>
 * @param <V>
 */
public abstract class AbstractTask<U, V> {
    /**
     * 任务名
     */
    protected String taskName = "AbstractTask";
    /**
     * 状态
     */
    ...
    public V sync(U request) {
        return TidyExecutor.instance().sync(this, request);
    }
    ...
    public Future<V> async(U request) {
        return TidyExecutor.instance().async(this, request);
    }
    ...
    public V get(U request, Future<V> future) {
        return TidyExecutor.instance().get(this, request, future);
    }
    ...
    public void delay(U request) {
        TidyExecutor.instance().delay(this, request);
    }
     ...
     * 自定义回调处理
     */
    protected abstract V callback(U request);
    ...
    public String toString() {
        return String.format(
                ...
                taskName, status, timeoutSeconds, timeoutPolicy, retryCount, retryDelaySeconds, retryLogic, retriedCount);
    }
